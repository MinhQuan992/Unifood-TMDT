...
<div id="detail">
    ...
    <c:if test="${not empty shipperName}">
        <p><strong>Tên shipper:</strong> <span><c:out value="${shipperName}"/></span></p>
    </c:if>
    <h2><strong>Tổng giá trị:</strong> <span style="font-weight: bold; color: red"><c:out value="${totalCost}"/></span></h2>

    <c:if test="${orderStatus == 'Đã tiếp nhận'}">
        <form class="frmEdit" method="get" action="${pageContext.request.contextPath}/editOrder">
            ...
            <input id="btnDuyet" type="submit" value="Duyệt đơn hàng">
        </form>
        ...
        <form class="frmEdit" method="get" action="${pageContext.request.contextPath}/editOrder">
            ...
            <input id="btnHuy" type="submit" value="Hủy đơn hàng">
        </form>
        <br>
        <br>
    </c:if>
    
    <c:if test="${orderStatus == 'Đang xử lý'}">
        <form class="frmEdit" method="get" action="${pageContext.request.contextPath}/editOrder">
            <input type="hidden" name="orderID" value="<c:out value="${orderID}"/>">
            <input type="hidden" name="customerEmail" value="<c:out value="${customerEmail}"/>">
            <input type="hidden" name="action" value="BanGiaoVanChuyen">
            <input id="btnBanGiaoVanChuyen" type="submit" value="Bàn giao vận chuyển">
        </form>
        <br>
        <br>
    </c:if>

    <a id="link-order" href="${pageContext.request.contextPath}/Page/OrderManage.jsp"><span><<</span>Quay lại danh sách đơn hàng</a>
</div>
<div id="footer">
